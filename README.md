...
are `-lua-max-memory 256`, and if you're running qw through a WebTiles or
...
* To run qw locally use a command like:
  ```bash
  ./crawl -lua-max-memory 256 -rc /path/to/qw/qw.rc -rcdir /path/to/qw
  ```
...
* To run qw locally use a command like:
  ```bash
  ./crawl -lua-max-memory 256 -rc /path/to/qw/qw-final.rc
  ```
